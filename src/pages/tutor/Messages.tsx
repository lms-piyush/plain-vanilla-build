...
const Messages = () => {
  ...
  const { data: conversations = [], isLoading } = useTutorConversations();
                  ...
                  >
                    <Avatar>
                      ...
                      <AvatarFallback className="bg-[#8A5BB7] text-white">
                        {conversation.student_profile?.full_name?.charAt(0) || 'S'}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline">
                        <h3 className="font-medium text-sm truncate">
                          {conversation.student_profile?.full_name || 'Unknown Student'}
                        </h3>
              ...
              <div className="p-4 border-b flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage src="" />
                    <AvatarFallback className="bg-[#8A5BB7] text-white">
                      {activeConversation.student_profile?.full_name?.charAt(0) || 'S'}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <h2 className="font-medium">
                      {activeConversation.student_profile?.full_name || 'Unknown Student'}
                    </h2>
                ...
                <div className="p-4 flex flex-col gap-4">
                  {messages.length === 0 ? (
                    ...
                    messages.map(message => (
                      <div 
                        key={message.id}
                        className={`flex ${message.sender_id === activeConversation.tutor_id ? 'justify-end' : 'justify-start'}`}
                      >
                        <div className={`flex ${message.sender_id === activeConversation.tutor_id ? 'flex-row-reverse' : 'flex-row'} items-start gap-2 max-w-[80%]`}>
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className={`text-xs text-gray-500 mt-1 ${message.sender_id === activeConversation.tutor_id ? 'text-right mr-2' : 'ml-2'}`}>
